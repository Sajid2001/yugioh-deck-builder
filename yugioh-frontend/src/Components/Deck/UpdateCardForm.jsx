...
export default function UpdateCardForm({handleFormOpen, cardQuantity, card}) {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full max-w-xs min-w-full my-3">
      <form onSubmit={handleSubmit} className="bg-amber-200 shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2" for="username">
            Edit Quantity
          </label>
          <span className='flex flex-row justify-center items-center'>
            <button type='button' onClick={() => changeQuantity(quantity - 1)} className='p-2 w-8 font-bold text-white bg-orange-500 hover:bg-orange-700 m-2 rounded-lg transition-duration-200 transition-all ease-in'>-</button>
            {quantity}
            <button type='button' onClick={() => changeQuantity(quantity + 1)} className='p-2 w-8 font-bold text-white bg-orange-500 hover:bg-orange-700 m-2 rounded-lg transition-duration-200 transition-all ease-in'>+</button>
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <button className="mr-8 bg-transparent hover:bg-orange-500 text-orange-500 transition-all transition-duration-200 ease-in font-semibold hover:text-white py-2 px-4 border border-orange-500 hover:border-transparent rounded" type="submit">
            Edit
          </button>
