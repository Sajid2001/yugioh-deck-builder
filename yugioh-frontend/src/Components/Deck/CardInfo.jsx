      ...
      <div className='flex flex-col justify-center items-center w-1/2'>
        ...
        { !formOpened &&
          <div className='flex flex-col items-center my-4 h-1/2'>
            ...
            <button onClick={handleFormOpen} className='bg-blue-400 p-3 rounded-lg w-2/3'>Edit</button>
          </div>
        }
          {formOpened && 
          <div className='h-1/2'>
        ...
        <div className='flex flex-col p-3 items-center'>
          <div className='bg-slate-100 p-3 rounded-lg px-6'>
          ...
          <p className='text-center font-light text-xl my-4 bg-slate-100 p-3 max-w-md rounded-lg'>{card.desc}</p>
          <div className='flex flex-row justify-center items-center m-4'>
            {card.prices && card.prices.map(price => (
              <span key={price.id} className='inline-block bg-slate-100 rounded-lg px-3 py-1 text-xl font-semibold text-gray-700 mr-2 mb-2'>{price.location}: ${price.price}</span>
            ))}
          </div>
        </div>
      </div>
